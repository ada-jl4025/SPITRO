...
// Get disruptions only
export async function getDisruptions(request: NextRequest) {
  try {
    const searchParams = request.nextUrl.searchParams;
    const modes = searchParams.get('mode')?.split(',').filter(Boolean);

    const disruptions = await tflClient.getDisruptions(modes);

    // Format disruptions
    const formattedDisruptions = disruptions
      .filter(line => line.disruptions && line.disruptions.length > 0)
      .map(line => ({
        lineId: line.id,
        lineName: line.name,
        modeName: line.modeName,
    ...
    return NextResponse.json<ApiResponse>({
      ...
      data: {
        lastUpdated: new Date().toISOString(),
        totalDisruptions: formattedDisruptions.reduce((acc, line) => acc + line.disruptions.length, 0),
        affectedLines: formattedDisruptions.length,
        disruptions: formattedDisruptions,
      },
    });

  } catch (error) {
    console.error('Disruptions check error:', error);
